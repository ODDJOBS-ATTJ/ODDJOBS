...
exports.create = (req, res) => {
  dbConn.query(
    "SELECT * FROM accounts WHERE userID = ? AND isWorker = 1",
    req.body.userID,
    (err, account) => {
      if (err) {
        ...
        res
          .status(400)
          .send({ error: true, message: "Account is not a worker" });
      } else {
...
exports.registerWorker = (req, res) => {
  const newWorker = {
    ...req.body,
    document1: req.files.fileInput1 ? req.files.fileInput1[0].path : null,
    document2: req.files.fileInput2 ? req.files.fileInput2[0].path : null,
    document3: req.files.fileInput3 ? req.files.fileInput3[0].path : null,  
  };
  Worker.create(newWorker, (err, worker) => {
    if (err) {
      res.send(err);
