...
function ServiceEdit() {
  ...
  const id = queryParams.get('serviceID'); // Get the service ID from the query parameters

  useEffect(() => {
    // Fetch the existing service data when the component is mounted
    axios.get(`http://localhost:3000/service/${id}`)
      .then(response => {
        ...
        setFormData({
          ...
          specifics: service.specifics.split(','), // Convert specifics string to array
  ...
  const handleSubmit = (e) => {
    ...
    if (window.confirm('Are you sure you want to Save this service?')) {
      const data = {
      ...
      axios.put(`http://localhost:3000/service/${id}`, data, {
        headers: {
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <button type="submit" className={styles["create-button"]}>SAVE SERVICE</button>
